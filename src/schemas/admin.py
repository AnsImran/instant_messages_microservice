...
class SettingsSnapshot(BaseSchema):
    ...
    queue_persistence_enabled:        bool  = Field(..., description="Whether the per-webhook queue is mirrored to a durable SQLite outbox.")
    queue_db_path:                    str   = Field(..., description="SQLite outbox path (restart-only).")
    queue_max_attempts:               int   = Field(..., description="Max replay attempts across restarts before a persisted item is dropped as poison.")
